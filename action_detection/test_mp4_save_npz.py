...
# Modified to support image/video input, MP4 rendering, per-person tracking, and per-track NPZ export.
...
class SimpleIOUTracker:
    """
    Lightweight multi-person tracker for board-side deployment.

    - Matching: greedy IoU assignment
    - State: bbox / kpts / score / lost count
    - History: each observed frame appended under the track id
    """
    def __init__(self, iou_thres=0.3, max_lost=30):
        self.iou_thres = float(iou_thres)
        self.max_lost = int(max_lost)
        self.next_track_id = 0
        self.tracks = {}   # track_id -> state dict
        self.history = {}  # track_id -> history dict
    ...
    def _new_track(self, det, frame_idx):
        ...
        self.history[track_id] = {
            ...
            'bboxes': [np.asarray(det['bbox'], dtype=np.float32)],
            'keypoints': [np.asarray(det['kpts'], dtype=np.float32)],
            'scores': [float(det['score'])],
            'class_ids': [int(det['class_id'])],
        }
        return track_id

    def _append_history(self, track_id, det, frame_idx):
        self.history[track_id]['frames'].append(int(frame_idx))
        self.history[track_id]['bboxes'].append(np.asarray(det['bbox'], dtype=np.float32))
        self.history[track_id]['keypoints'].append(np.asarray(det['kpts'], dtype=np.float32))
        self.history[track_id]['scores'].append(float(det['score']))
        self.history[track_id]['class_ids'].append(int(det['class_id']))

    def update(self, detections, frame_idx):
        """
        detections: list of dicts with keys bbox, score, class_id, kpts
        returns: list of dicts with an added key track_id
        """
        active_track_ids = [tid for tid, st in self.tracks.items() if st['lost'] <= self.max_lost]
        candidate_pairs = []
        ...
        for iou, tid, det_idx in candidate_pairs:
        ...
        # unmatched existing tracks become more lost
        ...
        for det_idx, det in enumerate(detections):
            if det_idx in det_to_track:
                ...
                self.tracks[tid]['bbox'] = np.asarray(det['bbox'], dtype=np.float32)
                self.tracks[tid]['kpts'] = np.asarray(det['kpts'], dtype=np.float32)
                self.tracks[tid]['score'] = float(det['score'])
                self.tracks[tid]['class_id'] = int(det['class_id'])
                self.tracks[tid]['lost'] = 0
                self.tracks[tid]['last_frame'] = int(frame_idx)
                self._append_history(tid, det, frame_idx)
            else:
                tid = self._new_track(det, frame_idx)
            det2 = dict(det)
            det2['track_id'] = int(tid)
            enriched.append(det2)

        # remove dead tracks from active state, keep history intact
        dead_ids = [tid for tid, st in self.tracks.items() if st['lost'] > self.max_lost]
        for tid in dead_ids:
            del self.tracks[tid]

        enriched.sort(key=lambda d: d['track_id'])
        return enriched
...
class Ultralytics_YOLO_Pose_Bayese_YUV420SP():
    ...
    def postProcess(self, outputs):
        ...
        if len(dbboxes) == 0 or sum(len(x) for x in dbboxes) == 0:
        ...
        for i in range(self.CLASSES_NUM):
            ...
            if np.sum(id_indices) == 0:
                continue
            indices = cv2.dnn.NMSBoxes(xyhw2[id_indices, :].tolist(), scores[id_indices].tolist(), self.SCORE_THRESHOLD, self.NMS_THRESHOLD)
            if len(indices) == 0:
                continue
            ...
            for indic in indices:
                ...
                x1 = int(np.clip(x1, 0, self.img_w))
                x2 = int(np.clip(x2, 0, self.img_w))
                y1 = int(np.clip(y1, 0, self.img_h))
                y2 = int(np.clip(y2, 0, self.img_h))
                ...
                for j in range(self.nkpt):
                    ...
                    kpts_.append((kpt_x, kpt_y, float(kpt_score)))

                results.append((i, float(scores[id_indices][indic]), int(x1), int(y1), int(x2), int(y2), kpts_))
        logger.debug("\033[1;31m" + f"Post Process time = {1000 * (time() - begin_time):.2f} ms" + "\033[0m")
        return results
...
def draw_detection(img, bbox, score, class_id, track_id=None):
    x1, y1, x2, y2 = bbox
    color = rdk_colors[(track_id if track_id is not None else class_id) % 20]
...
def draw_pose(img, kpts, kpt_conf_thres=0.5, track_id=None):
    kpt_conf_inverse = -np.log(1 / kpt_conf_thres - 1)
    line_color = (0, 255, 0) if track_id is None else rdk_colors[track_id % 20]
    ...
    for a, b in POSE_SKELETON:
        ...
        if score_a >= kpt_conf_inverse and score_b >= kpt_conf_inverse:
            cv2.line(img, (xa, ya), (xb, yb), line_color, 2)

    for j in range(len(kpts)):
        x, y = int(kpts[j][0]), int(kpts[j][1])
        if kpts[j][2] < kpt_conf_inverse:
            continue
        pt_color = (0, 0, 255) if track_id is None else rdk_colors[track_id % 20]
        cv2.circle(img, (x, y), 4, pt_color, -1)
...
def render_tracked_results(frame, tracked_results, kpt_conf_thres):
    for det in tracked_results:
        x1, y1, x2, y2 = [int(v) for v in det['bbox']]
        draw_detection(frame, (x1, y1, x2, y2), det['score'], det['class_id'], det.get('track_id', None))
        draw_pose(frame, det['kpts'], kpt_conf_thres, det.get('track_id', None))
    return frame


def save_tracks_npz(npz_path, tracker, fps, width, height, input_path, min_track_length=1):
    ...
    kept_ids = []
    for track_id in sorted(tracker.history.keys()):
        hist = tracker.history[track_id]
        if len(hist['frames']) < int(min_track_length):
            continue
        kept_ids.append(track_id)
        prefix = f'track_{track_id:04d}'
        export[f'{prefix}_frames'] = np.asarray(hist['frames'], dtype=np.int32)
        export[f'{prefix}_bboxes'] = np.asarray(hist['bboxes'], dtype=np.float32)
        # convert keypoint conf from raw logit to probability before saving
        kpts = np.asarray(hist['keypoints'], dtype=np.float32)
        if kpts.size > 0:
            kpts[..., 2] = sigmoid(kpts[..., 2])
        export[f'{prefix}_keypoints'] = kpts
        export[f'{prefix}_scores'] = np.asarray(hist['scores'], dtype=np.float32)
        export[f'{prefix}_class_ids'] = np.asarray(hist['class_ids'], dtype=np.int32)

    export['track_ids'] = np.asarray(kept_ids, dtype=np.int32)
    np.savez_compressed(npz_path, **export)
    logger.info("\033[1;32m" + f"saved per-track npz in path: {npz_path}" + "\033[0m")
    logger.info(f"saved {len(kept_ids)} tracks")


def infer_image(model, input_path, output_path, kpt_conf_thres, npz_save_path='',
                track_iou_thres=0.3, max_lost=30, min_track_length=1):
    img = cv2.imread(input_path)
    if img is None:
        raise ValueError(f"Load image failed: {input_path}")

    results = infer_single_frame(model, img)
    tracker = SimpleIOUTracker(iou_thres=track_iou_thres, max_lost=max_lost)
    tracked = tracker.update(results_to_detections(results), frame_idx=0)

    logger.info("\033[1;32m" + "Draw Results: " + "\033[0m")
    render_tracked_results(img, tracked, kpt_conf_thres)
    ...
    if npz_save_path:
        save_tracks_npz(npz_save_path, tracker, fps=1.0, width=img.shape[1], height=img.shape[0],
                        input_path=input_path, min_track_length=min_track_length)



def infer_video(model, input_path, output_path, kpt_conf_thres, show_fps=False, npz_save_path='',
                track_iou_thres=0.3, max_lost=30, min_track_length=1):
    ...
    while True:
        ...
        tracked = tracker.update(results_to_detections(results), frame_idx=frame_idx)
        render_tracked_results(frame, tracked, kpt_conf_thres)

        if show_fps:
            avg_fps = 1000.0 / (total_infer / max(frame_idx + 1, 1)) if total_infer > 0 else 0.0
            text = f"frame={frame_idx} infer={infer_ms:.1f}ms avg_fps={avg_fps:.2f} tracks={len(tracker.history)}"
            cv2.putText(frame, text, (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)

        writer.write(frame)
        if frame_idx % 10 == 0:
            logger.info(f"processed frame {frame_idx}, total tracks={len(tracker.history)}")
        frame_idx += 1
    ...
    if npz_save_path:
        save_tracks_npz(npz_save_path, tracker, fps=fps, width=width, height=height,
                        input_path=input_path, min_track_length=min_track_length)
...
def main():
    ...
    parser.add_argument('--npz-save-path', type=str, default='',
                        help='Optional path to save tracked pose sequences as npz, split by track id.')
    ...
    parser.add_argument('--track-iou-thres', type=float, default=0.3,
                        help='IoU threshold used for associating detections to existing tracks.')
    parser.add_argument('--max-lost', type=int, default=20,
                        help='How many consecutive missed frames a track can survive.')
    parser.add_argument('--min-track-length', type=int, default=1,
                        help='Only save tracks with at least this many observed frames into npz.')
    ...
    if suffix in ['.jpg', '.jpeg', '.png', '.bmp', '.webp']:
        infer_image(model, opt.input, opt.output, opt.kpt_conf_thres, opt.npz_save_path,
                    track_iou_thres=opt.track_iou_thres, max_lost=opt.max_lost,
                    min_track_length=opt.min_track_length)
    elif suffix in ['.mp4', '.avi', '.mov', '.mkv', '.m4v']:
        infer_video(model, opt.input, opt.output, opt.kpt_conf_thres, opt.show_fps, opt.npz_save_path,
                    track_iou_thres=opt.track_iou_thres, max_lost=opt.max_lost,
                    min_track_length=opt.min_track_length)
    else:
